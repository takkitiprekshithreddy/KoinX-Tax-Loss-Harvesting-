// src/data/mockApi.js
// Async fetch functions that simulate real API behavior (loading delay + optional error state)

const RAW_CAPITAL_GAINS = {
  capitalGains: {
    stcg: {
      profits: 70200.88,
      losses: 1548.53,
...
const RAW_HOLDINGS = [
  ...
  },
  {
    ...
    stcg: { balance: 2.75145540184285, gain: -3240.75 },
    ltcg: { balance: 0, gain: 0 },
  },
  {
    coin: "GONE",
  ...
  },
  {
    ...
    stcg: { balance: 0.0004211938732637162, gain: -5180.50 },
    ltcg: { balance: 0, gain: -420.00 },
  },
  {
    coin: "QUICK",
...
// Simulate network delay between min and max ms
const delay = (min = 600, max = 900) =>
  new Promise((resolve) =>
 ...
 * @param {boolean} shouldFail - Pass true to simulate an API error (dev/test use)
 */
export const fetchCapitalGains = async (shouldFail = false) => {
  await delay();
  if (shouldFail) throw new Error("Failed to fetch capital gains data.");
  return RAW_CAPITAL_GAINS;
};
 ...
 * @param {boolean} shouldFail - Pass true to simulate an API error (dev/test use)
 */
export const fetchHoldings = async (shouldFail = false) => {
  await delay();
  if (shouldFail) throw new Error("Failed to fetch holdings data.");
  return RAW_HOLDINGS;
};

// Keep legacy named exports so nothing else breaks
export const capitalGainsMock = RAW_CAPITAL_GAINS;
export const holdingsMock = RAW_HOLDINGS;
